// PR Gate Check — deterministic, comment-only bot.
//
...
const MARKER = '<!-- pr-gate-check:report -->';
...
module.exports = async ({ github, context, core }) => {
  ...
  const header = failed.length > 0
    ? `### ⚠️ PR gate check — ${failed.length} issue${failed.length > 1 ? 's' : ''} found`
    : unknown.length > 0
      ? '### ⏳ PR gate check — mostly clear, one check still pending'
      : '### ✅ PR gate check — all automated checks pass';
  ...
  const footer =
    '\n\n*This is an automated, comment-only check — it never closes or approves a PR. ' +
  ...
  core.info(`${failed.length} failing / ${unknown.length} pending / ${findings.length - failed.length - unknown.length} passing gate(s) for PR #${prNumber}.`);
};
